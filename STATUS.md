...
- BetterCodex now generates its own app icon during bundling: `CFBundleIconFile` is `bettercodex.icns`, the installed icon file exists at `/Applications/BetterCodex.app/Contents/Resources/bettercodex.icns`, and Quick Look rendered the preview with the Codex base plus a teal plugin-grid accessory badge.
